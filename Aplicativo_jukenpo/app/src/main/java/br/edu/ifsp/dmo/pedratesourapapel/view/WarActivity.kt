...
class WarActivity : AppCompatActivity(){
    ...
    private fun proclaimWinner(){
        val str = "${war.getWinner().nome}${getString(R.string.won_the_march)}"
    ...
    private fun startSelectionActivity(number: Int){
        val name = if (number == 1) war.opponent1.nome else war.opponent2.nome
    ...
    private fun updateUI() {
        val str = "${war.opponent1.nome} X ${war.opponent2.nome}"
        actionBar?.setTitle(str)
        binding.labelPlayer1.text = war.opponent1.nome
        binding.labelPlayer2.text = war.opponent2.nome
        updateScoreBoard()

        binding.buttonWeapon1.text = "${war.opponent1.nome} ${getString(R.string.gum_selection)}"
        binding.buttonWeapon2.text = "${war.opponent2.nome} ${getString(R.string.gum_selection)}"
    }
}